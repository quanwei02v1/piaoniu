    ...
    <div class="star_array">
      <div class="stars_head">
        ...
        <div class="stars_detail">
          <img :src="activityLineup.avatar" alt />
          <div class="item_box">
            <div class="star-item_main">{{activityLineup.name}}</div>
            <div class="star-item__focus">
              <div class="star_people">
                <img class="item__focus" 
                v-for="(item,index) in fans" :key='index'
                :src="item.avatar ? item.avatar :'./img/1344.png'" />
              </div>
              <div class="focus_bnt">
                <span>{{activityLineup.followNum}}</span>人关注
              </div>
              <div class="item_btn">
...
export default {
 ...
 async created() {
      ...
      this.star = Math.floor(data.star/2);//星星数量
      this.activityLineup = data.activityLineup[0]//艺人详情
      this.actorsId=this.activityLineup.id //艺人id
      this.fans= this.activityLineup.fans //艺人粉丝信息
      document.title = data.shortname //title短标题 
    },
  data() {
    return {
      imgCons:"",
